...
Based on findings within this report, we advise increased travel advertisement during the months leading up to September and December, an inquiry into the Qantas Airways buisness model, maintenance checks for route SYD-AKL, and research into what makes Australian-New Zealand relations so successful.
...
Let's move onto exploring the data. What are the most popular months for travel within Australia? We want to increase advertising leading up to these months to maximize potential revenue.
...
What airlines are most successful? We want to study their business models and understand why flyers choose certain airlines over others operating in their home country. See word cloud below for an overall visual understanding and the table of the top 10 most successful airlines for a more concrete understanding.
```{r, echo=FALSE}
...
```
We can see that some of the highest performing airlines are `Qantas Airways`, `Jetstar`, `Virgin Australia`, `Air New Zealand`, `Emirates`, `Cathay Pacific Airways`, and `Garuda Indonesia`.


What are the top five most frequent routes? We want to watch maintenance checks for planes flying these routes in particular and ensure availability year round for these routes.
```{r, echo=FALSE, results="hide"}
...
What international countries visit Australia most frequently? We want to create a personalised experience for our most frequent customer base and understand how to attract flyers from other countries.
...
We want to ensure that airlines are offering high seat availability for the most frequent routes to maximize profit. Let's explore through an independence test.
...
We observe a p-value of `2.2e-16` which is extremely far below 0.05. Therefore our *statistical conclusion* is that we reject H0 as p < 0.05 and our *scientific conclusion* is that we can say with almost no doubt that most frequent route is not independent of seat number meaning they have some relationship. See relationship visualised below.
...
the residual plot demonstrates a clear trend so we deem the model to be unreliable. Let's switch to exploring through a line plot.
...
We can see an `increasing trend` until late 2017 when COVID dramatically reduced the number of flights taken. This indicates the relative success of the airlines industry and raises the question of how we should encourage international travel to Australia post COVID.
